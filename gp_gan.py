...
def gp_gan(obj, bg, mask, gan_im_tens, inputdata, sess, image_size, color_weight=1, sigma=0.5, gradient_kernel='normal', smooth_sigma=1):
    ...
    copy_paste_init = preprocess(copy_paste_init)

    with sess.as_default():
        gan_im = sess.run(gan_im_tens, feed_dict={inputdata: [copy_paste_init]})
    gan_im = np.clip((np.squeeze(gan_im) + 1) / 2, 0, 1).astype(obj.dtype)
